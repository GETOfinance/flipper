...
import { RISK_LEVELS, RISK_COLORS, AGENT_TIERS, CONTRACTS, SEPOLIA_USDT_ADDRESS } from "../lib/constants";
import { useLiveMarketData } from "../lib/useLiveMarket";
import AgentSimulation from "../components/AgentSimulation";
import toast from "react-hot-toast";
import { getOrCreatePinnedWDKWallet } from "../lib/wdkWallet";
import { formatUSDT, getUSDTContract, parseUSDT } from "../lib/usdt";
import {
  Shield,
  Activity,
  AlertTriangle,
...
// ─── Fallback Data (displayed only when Ethereum Sepolia RPC is unreachable) ─────
...
export default function Home() {
  const { address, isConnected, connect, disconnect, isConnecting, switchToSepolia, chainId, provider, signer } = useWallet();
  ...
  // WDK wallet pinned to the connected MetaMask address
  const wdkWallet = useMemo(() => {
    if (!address) return null;
    return getOrCreatePinnedWDKWallet(address);
  }, [address]);

  const [wdkUsdtBalance, setWdkUsdtBalance] = useState<string>("0");
  const [wdkAmount, setWdkAmount] = useState<string>("");
  const [wdkTxs, setWdkTxs] = useState<{ hash: string; direction: "in" | "out"; amount: string; timestamp: number }[]>([]);
  const [wdkLastTelegramMessage, setWdkLastTelegramMessage] = useState<string>("");

  const TELEGRAM_USERNAME = process.env.NEXT_PUBLIC_TELEGRAM_USERNAME || "";
  const ETHERSCAN_API_KEY = process.env.NEXT_PUBLIC_ETHERSCAN_API_KEY || "";

  const refreshWdkBalance = useCallback(async () => {
    if (!provider || !wdkWallet) return;
    try {
      const usdt = getUSDTContract(provider);
      const bal: bigint = await usdt.balanceOf(wdkWallet.address);
      setWdkUsdtBalance(formatUSDT(bal));
    } catch {
      // keep previous
    }
  }, [provider, wdkWallet]);

  // Keep balance fresh on connect
  useEffect(() => {
    if (!isConnected) return;
    refreshWdkBalance();
  }, [isConnected, refreshWdkBalance]);

  const pushWdkTx = useCallback((tx: { hash: string; direction: "in" | "out"; amount: string }) => {
    setWdkTxs((prev) => [{ ...tx, timestamp: Date.now() }, ...prev].slice(0, 20));
  }, []);

  const refreshWdkTxHistory = useCallback(async () => {
    if (!wdkWallet) return;
    try {
      const qs = new URLSearchParams({
        ...
        contractaddress: SEPOLIA_USDT_ADDRESS,
        page: "1",
        offset: "20",
        sort: "desc",
      });
      if (ETHERSCAN_API_KEY) qs.set("apikey", ETHERSCAN_API_KEY);
      const url = `https://api-sepolia.etherscan.io/api?${qs.toString()}`;
      ...
      const rows: EtherscanTokenTx[] =
        typeof json === "object" && json !== null && Array.isArray((json as { result?: unknown }).result)
          ? ((json as { result: unknown[] }).result.filter((r): r is EtherscanTokenTx => {
              if (typeof r !== "object" || r === null) return false;
              const rr = r as Partial<EtherscanTokenTx>;
              return (
                typeof rr.hash === "string" &&
                typeof rr.from === "string" &&
                typeof rr.to === "string" &&
                typeof rr.value === "string" &&
                typeof rr.tokenDecimal === "string" &&
                typeof rr.timeStamp === "string"
              );
            }) as EtherscanTokenTx[])
          : [];
      ...
      const normalized = rows.map((r) => {
        ...
        const direction: "in" | "out" = to === wdkWallet.address.toLowerCase() ? "in" : "out";
        const value = BigInt(r.value);
        const decimals = Number(r.tokenDecimal || "6");
        const amount = ethers.formatUnits(value, decimals);
        const tsMs = Number(r.timeStamp) * 1000;
        return {
          hash: r.hash,
          direction,
          amount,
      ...
      setWdkTxs((prev) => {
        const map = new Map<string, { hash: string; direction: "in" | "out"; amount: string; timestamp: number }>();
        for (const tx of [...normalized, ...prev]) map.set(tx.hash, tx);
        return Array.from(map.values()).sort((a, b) => b.timestamp - a.timestamp).slice(0, 20);
      });
    } catch {
      // ignore
    }
  }, [wdkWallet, ETHERSCAN_API_KEY]);

  useEffect(() => {
    if (!wdkWallet) return;
    refreshWdkTxHistory();
    const t = setInterval(refreshWdkTxHistory, 30000);
    return () => clearInterval(t);
  }, [wdkWallet, refreshWdkTxHistory]);

  const setTelegramMessageForTx = useCallback((txHash: string, direction: "in" | "out", amount: string) => {
    const toUser = TELEGRAM_USERNAME ? `@${TELEGRAM_USERNAME.replace(/^@/, "")}` : "(no telegram username configured)";
    const msg = `WDK USDT ${direction === "in" ? "fund" : "transfer"}: ${amount} USDT | tx ${txHash} | to ${toUser}`;
    setWdkLastTelegramMessage(msg);
  }, [TELEGRAM_USERNAME]);

  const handleFundWDK = async () => {
    if (!isConnected || !signer || !address || !wdkWallet) {
      toast.error("Connect wallet first");
      return;
    }
    if (!wdkAmount || parseFloat(wdkAmount) <= 0) {
      toast.error("Enter a valid USDT amount");
      return;
    }
    try {
      toast.loading("Funding WDK wallet...", { id: "wdk-fund" });
      const usdt = getUSDTContract(signer);
      const amount = parseUSDT(wdkAmount);

      // Preflight balance check to avoid estimateGas revert:
      // "ERC20: transfer amount exceeds balance"
      const bal: bigint = await usdt.balanceOf(address);
      if (bal < amount) {
        toast.error(`Insufficient USDT balance. Have ${formatUSDT(bal)} USDT, need ${wdkAmount} USDT`, {
          id: "wdk-fund",
        });
        return;
      }

      const tx = await usdt.transfer(wdkWallet.address, amount);
      const receipt = await tx.wait();
      toast.success("WDK funded", { id: "wdk-fund" });
      const hash = receipt?.hash || tx.hash;
      pushWdkTx({ hash, direction: "in", amount: wdkAmount });
      setTelegramMessageForTx(hash, "in", wdkAmount);
      setWdkAmount("");
      refreshWdkBalance();
    } catch (err: unknown) {
      const msg = err instanceof Error ? err.message : "Funding failed";
      toast.error(msg, { id: "wdk-fund" });
    }
  };

  const handleTransferFromWDK = async () => {
    if (!isConnected || !provider || !address || !wdkWallet) {
      toast.error("Connect wallet first");
      return;
    }
    if (!wdkAmount || parseFloat(wdkAmount) <= 0) {
    ...
    // Client-only signing with locally stored WDK private key
    try {
      toast.loading("Transferring from WDK...", { id: "wdk-transfer" });
      const wdkSigner = new ethers.Wallet(wdkWallet.privateKey, provider);
      const usdt = getUSDTContract(wdkSigner);
      const amount = parseUSDT(wdkAmount);

      // Preflight balance check to avoid estimateGas revert:
      const bal: bigint = await usdt.balanceOf(wdkWallet.address);
      if (bal < amount) {
        toast.error(`WDK wallet has insufficient USDT. Have ${formatUSDT(bal)} USDT, need ${wdkAmount} USDT`, {
          id: "wdk-transfer",
        });
        return;
      }
      ...
      const receipt = await tx.wait();
      toast.success("Transferred to connected wallet", { id: "wdk-transfer" });
      const hash = receipt?.hash || tx.hash;
      pushWdkTx({ hash, direction: "out", amount: wdkAmount });
      setTelegramMessageForTx(hash, "out", wdkAmount);
      setWdkAmount("");
      refreshWdkBalance();
    } catch (err: unknown) {
      const msg = err instanceof Error ? err.message : "Transfer failed";
      toast.error(msg, { id: "wdk-transfer" });
    }
  };
    ...
    <div className="min-h-screen">
      ...
      <nav className="glass-card mx-4 mt-4 px-6 py-4 flex items-center justify-between" style={{ borderRadius: "12px" }}>
        ...
        <div className="flex items-center gap-4">
          ...
          {isConnected ? (
            <div className="flex items-center gap-3">
              {chainId !== 11155111 && (
                <button onClick={switchToSepolia} className="text-sm text-yellow-400 hover:text-yellow-300 transition-colors">
                  Switch to Sepolia
                </button>
              )}
      ...
      <section className="px-4 pt-16 pb-12 text-center">
        <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full mb-6" style={{ background: "rgba(0,224,255,0.08)", border: "1px solid rgba(0,224,255,0.15)" }}>
          ...
          {!isConnected || !wdkWallet ? (
            <span className="text-sm text-[#00e0ff]">Autonomous AI Agent powered by OpenClaw transacting with USD₮</span>
          ) : (
            <div className="flex flex-col md:flex-row md:items-center gap-2 text-left">
              <span className="text-xs text-gray-300">
                WDK Wallet:
                <span className="ml-2 font-mono text-[#00e0ff]">{wdkWallet.address.slice(0, 8)}...{wdkWallet.address.slice(-6)}</span>
              </span>
              <span className="text-xs text-gray-400 md:ml-3">USDT: <span className="font-mono text-white">{wdkUsdtBalance}</span></span>
              <div className="flex items-center gap-2 md:ml-3">
                <input
                  value={wdkAmount}
                  onChange={(e) => setWdkAmount(e.target.value)}
                  placeholder="Amount (USDT)"
                  className="bg-black/30 border border-white/10 rounded-lg px-3 py-1.5 text-xs text-gray-200 w-32"
                />
                <button
                  onClick={handleFundWDK}
                  className="text-xs px-3 py-1.5 rounded-lg"
                  style={{ background: "rgba(34,197,94,0.10)", color: "#22c55e", border: "1px solid rgba(34,197,94,0.20)" }}
                  title="Fund WDK wallet with USDT from connected MetaMask"
                >
                  Fund
                </button>
                <button
                  onClick={handleTransferFromWDK}
                  className="text-xs px-3 py-1.5 rounded-lg"
                  style={{ background: "rgba(0,224,255,0.10)", color: "#00e0ff", border: "1px solid rgba(0,224,255,0.20)" }}
                  title="Transfer USDT from WDK wallet back to connected MetaMask"
                >
                  Transfer
                </button>
              </div>
        ...
        <p className="text-lg text-gray-400 max-w-2xl mx-auto mb-8">
          Flipper is an autonomous AI agent that monitors your DeFi positions on Ethereum 24/7,
        ...
        {!liveMarket.isLoading && liveMarket.ethPriceCoinGecko > 0 && (
          <div className="flex items-center justify-center gap-6 mb-8 flex-wrap">
            ...
            <div className="glass-card px-4 py-2 flex items-center gap-2" style={{ borderRadius: "10px" }}>
              <span className="text-xs text-gray-500">ETH TVL</span>
              <span className="text-sm font-mono text-[#f0b90b]">${(liveMarket.ethTvl / 1e9).toFixed(2)}B</span>
            </div>
            <div className="glass-card px-4 py-2 flex items-center gap-2" style={{ borderRadius: "10px" }}>
      ...
      <section className="px-4 pb-20">
        <div className="max-w-6xl mx-auto">
          ...
          <p className="text-center text-gray-400 mb-10 max-w-2xl mx-auto">
            Flipper combines LLM reasoning with on-chain DEX data for multi-layered threat detection that goes beyond simple heuristics.
          </p>
          ...
          <div className="grid md:grid-cols-2 gap-6 mb-8">
                ...
                <div className="p-4 rounded-xl" style={{ background: "rgba(0,0,0,0.3)" }}>
                  ...
                  <p className="text-sm text-gray-300 leading-relaxed italic">
                    {liveMarket.ethPriceCoinGecko > 0
                      ? `"ETH trading at $${liveMarket.ethPriceCoinGecko.toFixed(2)} with ${liveMarket.priceChange24h >= 0 ? "+" : ""}${liveMarket.priceChange24h.toFixed(2)}% 24h movement. Volume at $${(liveMarket.volume24h / 1e9).toFixed(2)}B. Ethereum ecosystem TVL at $${(liveMarket.ethTvl / 1e9).toFixed(2)}B. Oracle cross-check delta: ${liveMarket.priceDelta.toFixed(3)}%. ${liveMarket.priceDelta < 1 ? "No oracle manipulation detected. All metrics within normal parameters." : "WARNING: Price divergence detected between sources."}"`
                      : `"ETH trading at $2800.00 with +1.8% 24h movement. Volume at $780M (+15% change). Ethereum ecosystem liquidity at $4.2B. Risk engine scores: Liquidation 8/100, Volatility 22/100, Protocol 5/100. No significant threats detected."`
                    }
                  </p>
                </div>
                  ...
                  <div className="space-y-1">
                    {[
                      liveMarket.ethPriceCoinGecko > 0
                        ...
                        : "ETH stable at $2800.00 with +1.8% 24h change",
                      ...
                      liveMarket.ethTvl > 0
                        ? `ETH TVL: $${(liveMarket.ethTvl / 1e9).toFixed(2)}B — ecosystem ${liveMarket.ethTvl > 3e9 ? "strong" : "monitoring"}`
            ...
            <div className="glass-card glow-border p-6" style={{ borderRadius: "16px" }}>
              <h4 className="text-lg font-semibold mb-4 flex items-center gap-2">
                ...
                <span className="ml-auto text-xs px-2 py-1 rounded-md bg-yellow-500/10 text-yellow-400 border border-yellow-500/20">Ethereum Sepolia</span>
              </h4>
              <div className="space-y-4">
                <div className="p-4 rounded-xl" style={{ background: "rgba(0,0,0,0.3)" }}>
                  ...
                  <div className="space-y-2">
                    <div className="flex justify-between items-center">
                      ...
                      <span className="text-sm font-mono text-white">${liveMarket.ethPriceCoinGecko > 0 ? liveMarket.ethPriceCoinGecko.toFixed(2) : "600.00"}</span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="text-sm text-gray-400">Uniswap v2 (On-chain)</span>
                      <span className="text-sm font-mono text-white">${liveMarket.ethPriceUniswap > 0 ? liveMarket.ethPriceUniswap.toFixed(2) : "599.50"}</span>
                    </div>
                    <div className="h-px bg-gray-700 my-1" />
                    <div className="flex justify-between items-center">
                      <span className="text-sm text-gray-400">Price Delta</span>
                      <span className={`text-sm font-mono font-bold ${liveMarket.priceDelta > 1 ? "text-yellow-400" : "text-green-400"}`}>{liveMarket.ethPriceCoinGecko > 0 ? liveMarket.priceDelta.toFixed(3) : "0.083"}% {liveMarket.priceDelta > 1 ? "⚠" : "✓"}</span>
                    </div>
                  </div>
                </div>
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {["WETH", "USDC", "DAI", "USDT", "WBTC", "LINK", "UNI", "AAVE"].map((token) => (
      ...
      <section className="px-4 pb-20">
        <div className="max-w-6xl mx-auto">
          ...
          <p className="text-center text-gray-400 mb-10 max-w-xl mx-auto">
            All smart contracts are deployed on Ethereum Sepolia Testnet and verified via Etherscan for full source code transparency.
          </p>
            ...
            {[
              ...
              <div key={i} className="glass-card glow-border p-6 group" style={{ borderRadius: "16px" }}>
                ...
                <div className="flex gap-2">
                  ...
                  <a href={`https://sepolia.etherscan.io/address/${contract.address}#code`}
                    target="_blank" rel="noopener noreferrer"
                    className="flex items-center gap-1 text-xs px-3 py-1.5 rounded-lg transition-colors"
                    style={{ background: "rgba(34,197,94,0.08)", color: "#22c55e", border: "1px solid rgba(34,197,94,0.15)" }}>
                    <CheckCircle className="w-3 h-3" /> Etherscan Verified
                  </a>
                </div>
              </div>
            ))}
          ...
          <div className="grid md:grid-cols-3 gap-6">
            <div className="glass-card glow-border p-6 md:col-span-2" style={{ borderRadius: "16px" }}>
              <h4 className="text-lg font-semibold mb-4 flex items-center gap-2">
                ...
                On-Chain Transaction Evidence
                <span className="ml-auto text-xs px-2 py-1 rounded-md bg-green-500/10 text-green-400 border border-green-500/20">{wdkTxs.length} TXs</span>
              </h4>
              <p className="text-sm text-gray-400 mb-4">
                Live Sepolia transactions executed by the pinned Tether WDK wallet.
              </p>
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {wdkTxs.length === 0 ? (
                      <tr className="border-b" style={{ borderColor: "rgba(0,224,255,0.03)" }}>
                        <td className="px-4 py-4 text-gray-500 text-sm" colSpan={4}>
                          No WDK transactions yet.
                        </td>
                      </tr>
                    ) : (
                      wdkTxs.map((tx, i) => (
                        <tr key={tx.hash} className="border-b hover:bg-white/[0.02]" style={{ borderColor: "rgba(0,224,255,0.03)" }}>
                          <td className="px-4 py-2.5 text-gray-500 font-mono">{i + 1}</td>
                          <td className="px-4 py-2.5">
                            <span className={`text-xs px-2 py-0.5 rounded-md ${tx.direction === "in" ? "bg-green-500/10 text-green-400" : "bg-[#00e0ff]/10 text-[#00e0ff]"}`}>
                              {tx.direction === "in" ? "Fund" : "Transfer"}
                            </span>
                          </td>
                          <td className="px-4 py-2.5 text-gray-300 text-xs font-mono">{tx.amount} USDT</td>
                          <td className="px-4 py-2.5">
                            <a
            ...
            <div className="glass-card glow-border p-6" style={{ borderRadius: "16px" }}>
              <h4 className="text-lg font-semibold mb-4 flex items-center gap-2">
                <Cpu className="w-5 h-5 text-[#a855f7]" />
                WDK → Telegram Message
              </h4>
              <p className="text-sm text-gray-400 mb-4">
                {TELEGRAM_USERNAME ? (
                  <span>
                    Target: <span className="font-mono text-gray-200">@{TELEGRAM_USERNAME.replace(/^@/, "")}</span>
                  </span>
                ) : (
                  <span className="text-yellow-400">No telegram username configured (set NEXT_PUBLIC_TELEGRAM_USERNAME in .env)</span>
                )}
              </p>
              <div className="bg-black/30 border border-white/10 rounded-lg p-3 min-h-[110px]">
                {wdkLastTelegramMessage ? (
                  <p className="text-xs text-gray-200 whitespace-pre-wrap">{wdkLastTelegramMessage}</p>
                ) : (
                  <p className="text-xs text-gray-500">No transaction message sent.</p>
                )}
              </div>
              <p className="text-[11px] text-gray-500 mt-3">
                This UI panel reflects the message content that would be sent via OpenClaw Telegram integration when enabled.
              </p>
            </div>
          </div>
      ...
      <footer className="glass-card mx-4 mb-4 px-6 py-6 flex flex-col md:flex-row items-center justify-between gap-4" style={{ borderRadius: "12px" }}>
        <div className="flex items-center gap-2">
          ...
          <span className="text-gray-400">Flipper Protocol — Tether Hackathon Galactica: WDK Edition 1</span>
        </div>
        <div className="flex items-center gap-4 flex-wrap justify-center">
          <a href={`https://sepolia.etherscan.io/address/${CONTRACTS.REGISTRY}`} target="_blank" rel="noopener noreferrer" className="text-gray-500 hover:text-[#00e0ff] transition-colors flex items-center gap-1 text-sm">
            Registry <ExternalLink className="w-3 h-3" />
          </a>
          <a href={`https://sepolia.etherscan.io/address/${CONTRACTS.VAULT}`} target="_blank" rel="noopener noreferrer" className="text-gray-500 hover:text-[#a855f7] transition-colors flex items-center gap-1 text-sm">
            Vault <ExternalLink className="w-3 h-3" />
          </a>
          <a href={`https://sepolia.etherscan.io/address/${CONTRACTS.DECISION_LOGGER}`} target="_blank" rel="noopener noreferrer" className="text-gray-500 hover:text-[#22c55e] transition-colors flex items-center gap-1 text-sm">
            Logger <ExternalLink className="w-3 h-3" />
          </a>
          <span className="text-gray-600 text-sm">Ethereum</span>
        </div>
      </footer>
    </div>
  );
        ...
        <div className="space-y-3">
          {displayDecisions.length > 0 ? displayDecisions.map((d) => (
            ...
            <div className="text-center py-6 text-gray-500 text-sm">
              ...
              {isLive ? "No decisions logged yet on-chain." : "Connecting to Ethereum Sepolia..."}
            </div>
          )}
        </div>
      </div>
          ...
          <tbody>
            {displayDecisions.length > 0 ? displayDecisions.map((d) => (
              ...
              <tr><td colSpan={8} className="px-6 py-8 text-center text-gray-500 text-sm">
                {isLive ? "No decisions logged on-chain yet. Run the agent to generate real AI decisions." : "Connecting to Ethereum Sepolia RPC..."}
              </td></tr>
            )}
          </tbody>
        </table>
    ...
    <div className="grid md:grid-cols-2 gap-6">
      ...
      <div className="glass-card glow-border p-6 md:col-span-2" style={{ borderRadius: "16px" }}>
        ...
        <div className="grid md:grid-cols-3 gap-4">
          {[
            { name: "FlipperRegistry", desc: "ERC-721 agent identity NFTs with 4-tier permissions, reputation scoring (1-5), and on-chain performance metrics.", color: "#00e0ff" },
            { name: "FlipperVault", desc: "Non-custodial vault for ETH/ERC-20 with agent authorization, per-user risk profiles, and autonomous protection execution.", color: "#a855f7" },
