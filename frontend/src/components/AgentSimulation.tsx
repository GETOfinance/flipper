...
function generateObserveData(market: LiveMarketData) {
  if (market.ethPriceCoinGecko > 0) {
    return {
      lines: [
        `> fetch("coingecko/eth") → $${market.ethPriceCoinGecko.toFixed(2)}`,
        `> fetch("defillama/ethereum") → TVL $${(market.ethTvl / 1e9).toFixed(2)}B`,
      ...
      summary: `Data collected: ETH at $${market.ethPriceCoinGecko.toFixed(2)}, Ethereum TVL $${(market.ethTvl / 1e9).toFixed(2)}B`,
    };
  }
  return {
    lines: [
      '> fetch("coingecko/eth") → $2800.00',
      '> fetch("defillama/ethereum") → TVL $4.20B',
      '> fetch("uniswap/v2/price") → $2799.50',
      "> 24h Change: +1.82%",
      "> Volume: $0.78B | Market Cap: $92.1B",
    ],
    summary: "Data collected: ETH at $2800.00, Ethereum TVL $4.20B",
  };
}
...
function generateReasonData(market: LiveMarketData) {
  ...
  if (Math.abs(change) > 5 && delta > 1) {
    ...
    reasoning = `ALL CLEAR: Market stable with ETH at $${market.ethPriceCoinGecko > 0 ? market.ethPriceCoinGecko.toFixed(2) : "2800.00"}. 24h change ${change >= 0 ? "+" : ""}${change.toFixed(2)}% is within normal parameters. Ethereum TVL healthy at $${(market.ethTvl / 1e9).toFixed(2)}B. No threats detected.`;
  }
...
function generateVerifyData(market: LiveMarketData) {
  const cg = market.ethPriceCoinGecko > 0 ? market.ethPriceCoinGecko : 2800.00;
  const ps = market.ethPriceUniswap > 0 ? market.ethPriceUniswap : 2799.50;
  const delta = market.priceDelta > 0 ? market.priceDelta : 0.083;
  const status = delta < 1 ? "✓ CONSISTENT" : delta < 5 ? "⚠ DIVERGENCE" : "🚨 CRITICAL";

  return {
    lines: [
      `> Uniswap V2 Router: 0x7a250...5630`,
      `> getAmountsOut(1 WETH → USDT) = $${ps.toFixed(2)}`,
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-xs text-gray-500 mt-1">
            ...
            {market.ethPriceCoinGecko > 0 ? "live market data" : "simulated data"}
          </p>
        </div>
