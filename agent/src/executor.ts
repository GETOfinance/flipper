// ═══════════════════════════════════════════════════════════════
// Flipper Protocol — On-Chain Executor
// Executes protective actions and logs decisions to Ethereum
// ═══════════════════════════════════════════════════════════════
...
export class OnChainExecutor {
  ...
  constructor(config: ExecutorConfig, provider: ethers.JsonRpcProvider) {
    ...
    console.log("[Flipper Executor] Initialized");
  ...
  async logDecision(
    ...
    console.log(`[Flipper Executor] Logging decision: type=${decisionType} risk=${riskLevel} confidence=${threat.confidence}%`);
    ...
    try {
      ...
      console.log(`[Flipper Executor] Decision logged: ${receipt.hash}`);
  ...
  async logRiskSnapshot(snapshot: RiskSnapshot): Promise<string | null> {
    console.log(`[Flipper Executor] Logging risk snapshot: LIQ=${snapshot.liquidationRisk} VOL=${snapshot.volatilityRisk} PROTO=${snapshot.protocolRisk} SC=${snapshot.smartContractRisk}`);
    ...
    try {
      ...
      const receipt = await tx.wait();
      console.log(`[Flipper Executor] Risk snapshot logged: ${receipt.hash}`);
      return receipt.hash;
    } catch (error: any) {
  ...
  async executeProtection(
    ...
    console.log(`[Flipper Executor] Executing protection: ${action} for ${userAddress} value=${value}`);
    ...
    try {
      ...
      const receipt = await tx.wait();
      console.log(`[Flipper Executor] Protection executed: ${receipt.hash}`);
  ...
  private async updateStats(success: boolean, valueProtected: bigint): Promise<void> {
    ...
    try {
      ...
      await tx.wait();
      console.log("[Flipper Executor] Agent stats updated");
    } catch (error: any) {
      console.error("[Flipper Executor] Stats update failed:", error.message);
    }
  }
