// ═══════════════════════════════════════════════════════════════
// Flipper Protocol — Uniswap V2 Integration
// Real on-chain DEX price feeds and token analysis for Ethereum
// ═══════════════════════════════════════════════════════════════

import { ethers } from "ethers";

// Uniswap V2 Router on Ethereum Mainnet & Sepolia
...
export class UniswapProvider {
  ...
  /**
   * Get token price in USD via Uniswap Router
   * Routes: TOKEN → WETH → USDC
   */
  async getTokenPriceUSD(tokenAddress: string): Promise<number> {
    ...
    try {
      ...
      if (tokenAddress.toLowerCase() === ETH_TOKENS.WETH.toLowerCase()) {
        // WETH → USDC direct
  ...
  async getETHPrice(): Promise<number> {
    return this.getTokenPriceUSD(ETH_TOKENS.WETH);
  }
  ...
  async getPairData(token0Address: string, token1Address: string): Promise<PairData | null> {
    try {
      ...
      // Estimate USD liquidity (using USDC/USDT reserves if one side is stablecoin)
      let liquidityUSD = 0;
      const stables = [ETH_TOKENS.USDC.toLowerCase(), ETH_TOKENS.USDT.toLowerCase()];
      if (stables.includes(token0Address.toLowerCase())) {
        ...
        if (token0Address.toLowerCase() === ETH_TOKENS.WETH.toLowerCase()) {
          liquidityUSD = r0Formatted * ethPrice * 2;
        } else if (token1Address.toLowerCase() === ETH_TOKENS.WETH.toLowerCase()) {
  ...
  async getPortfolioPrices(tokens: string[]): Promise<TokenPrice[]> {
    ...
    for (const token of tokens) {
      try {
        ...
        const pairAddr = await this.factory.getPair(token, ETH_TOKENS.WETH).catch(() => ethers.ZeroAddress);
        ...
        if (pairAddr !== ethers.ZeroAddress) {
          ...
          const ethPrice = await this.getETHPrice();
          // Assume token1 is WETH (common on Uniswap)
  ...
  /**
   * Get total pairs on Uniswap (shows DEX depth)
   */
  ...
  async analyzeTokenRisk(tokenAddress: string): Promise<{
    ...
    const pairData = await this.getPairData(tokenAddress, ETH_TOKENS.WETH);
