...
async function main() {
  ...
  const Vault = await ethers.getContractFactory("FlipperVault");
  const vault = await Vault.deploy(registryAddress, protocolFeeBps, minDeposit);
  await vault.waitForDeployment();
  const vaultAddress = await vault.getAddress();
  console.log("   FlipperVault deployed to:", vaultAddress);
  ...
  console.log("\n5. Registering initial Flipper Guardian Agent...");
  const tx4 = await registry.registerAgent(
    "Flipper Guardian Alpha",
    "https://flipper-protocol.io/agent/alpha",
    3, // Archon tier (full autonomy)
    { value: registrationFee }
  );
  await tx4.wait();
  console.log("   ✓ Agent 'Flipper Guardian Alpha' registered (ID: 0)");
  ...
  console.log(`  FlipperRegistry:  ${registryAddress}`);
  console.log(`  FlipperVault:     ${vaultAddress}`);
  ...
  const deploymentData = {
    ...
    contracts: {
      FlipperRegistry: registryAddress,
      FlipperVault: vaultAddress,
